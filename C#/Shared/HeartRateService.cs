...
namespace BluetoothGattHeartRate
{
    ...
    public class HeartRateService
    {
        ...
        private Guid CHARACTERISTIC_UUID = GattCharacteristicUuids.RscMeasurement;
        ...
        {
            int flag = data[0];
            int instantaneousSpeed = data[1] | (data[2] << 8);
            int instantaneousCadence = data[3];
            int instantaneousStrideLength = -1;
            int totalDistance = -1;
            ...
            if ((flag & 0x01) != 0)
            {
                instantaneousStrideLength = data[index] | (data[index + 1] << 8);
                index += 2;
            }

            if ((flag & 0x02) != 0)
            {
                totalDistance = data[index] | (data[index + 1] << 8) | (data[index + 2] << 16) | (data[index + 3] << 24);
                index += 4;
            }

            //UIHelper.ExecuteOnUIThread(() => StrideSensorValuesChanged(instantaneousSpeed, instantaneousCadence, instantaneousStrideLength, totalDistance));
            // The Heart Rate Bluetooth profile can also contain sensor contact status information,
            // and R-Wave interval measurements, which can also be processed here. 
            // For the purpose of this sample, we don't need to interpret that data.
            ...
            return new HeartRateMeasurement
            {
                HeartRateValue = (ushort)instantaneousSpeed,
                //HasExpendedEnergy = hasEnergyExpended,
                ExpendedEnergy = (ushort)instantaneousCadence
            };
        }
